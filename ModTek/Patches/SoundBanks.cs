...
namespace ModTek {
  ...
  public static class LoadedAudioBank_LoadBankExternal {
    private static HashSet<string> failLoadedSoundBanks = new HashSet<string>();
    public static bool Prefix(LoadedAudioBank __instance, ref AKRESULT __result, ref uint ___id) {
      ...
      if (failLoadedSoundBanks.Contains(__instance.name)) {
        RLog.M.WL(1, "bank marked as corrupted. No loading performed");
      }
      ...
      if ((www.bytes[0] == 0x42) && (www.bytes[1] == 0x4B) && (www.bytes[2] == 0x48) && (www.bytes[3] == 0x44)) {
        pparams = null;
        RLog.M.WL(1, "processing no need");
      } else {
        pparams = SoundBanksProcessHelper.GetRegistredProcParams(__instance.name);
      }
      try {
        GCHandle? handle = null;
        uint dataLength = (uint)www.bytes.Length;
        if (pparams != null) {
          RLog.M.WL(1, "found post-process parameters");
          Aes aes = Aes.Create();
      ...
      if(__result == AKRESULT.AK_Success) {
        failLoadedSoundBanks.Remove(__instance.name);
      } else {
        failLoadedSoundBanks.Add(__instance.name);
      }
      return false;
    }
  }
